...
const prices = Object.fromEntries(
  marketSubgraphData.data.listings.map((e) => [
    e.tokenID,
    utils.formatEther(e.price),
  ])
);
       ...
       <tr>
        ...
        <td>
          <div style="font-size:60%">
            ${formatMultiplier(
              artifact.energyCapMultiplier
            )} ${formatMultiplier(
    artifact.energyGrowthMultiplier
  )} ${formatMultiplier(artifact.rangeMultiplier)} ${formatMultiplier(
    artifact.speedMultiplier
  ...
  return html`
    <div style="max-height:33%" overflow="scroll">
      ...
      <table>
        ...
        <tbody>
          ${Object.values(dfSubgraphData.data.myartifacts).map((artifact) => {
            return myRow(artifact);
          })}
        </tbody>
      </table>
    </div>
  ...
  return html`
    <div style="max-height:33%" overflow="scroll">
      ...
      <table>
        ...
        <tbody>
          ${Object.values(dfSubgraphData.data.shopartifacts).map((artifact) => {
            return saleRow(artifact);
          })}
        </tbody>
      </table>
    </div>
...
function App() {
  return html`
    <style>
      <!-- ideally all the css stuff goes here -- > table {
        table-layout: fixed;
        width: 100%;
...
class Plugin {
  ...
  async render(container) {
    ...
    if (this.approval != TOKENS_CONTRACT_ADDRESS) {
      await TOKENS.setApprovalForAll(SALES_CONTRACT_ADDRESS, true)
        .then((res) => {
